...
export default function CanvasGarden() {
  ...
  const [taskPanelClosedThisSession, setTaskPanelClosedThisSession] =
    useState(false);
  ...
  >(new Map());
  const [plantContextMenu, setPlantContextMenu] = useState<{
    plant: Plant;
    x: number;
    y: number;
  } | null>(null);
  ...
  const isBlockingModalOpen =
    showPlantCreator || showWorkDialog || showTrophyDialog;
  ...
  const handlePlantAction = (
    action: "info" | "tasks" | "manage" | "harvest",
    plant: Plant
  ) => {
  ...
  const startPlantGlow = (plantId: string) => {
    setPlants((prevPlants) =>
      prevPlants.map((plant) =>
        plant.id === plantId ? { ...plant, shouldGlow: true } : plant
      )
    );
  ...
  const logWork = async (plantId: string, hours: number) => {
    ...
    if (plantBefore?.is_multi_step) {
      setSubmissionMessage(
        `Time logged! +${xpGained} XP gained. Use "Complete Step" to mark steps as done.`
      );
    } else {
    ...
    try {
      ...
      setPlants((prevPlants) =>
        prevPlants.map((plant) => {
          if (plant.id === plantId) {
            return {
              ...plant,
              experience_points: workResult.new_growth_level
                ? (workResult.new_growth_level / 20) * 100
      ...
      if (stageAfter > stageBefore) {
        ...
        if (plantAfter) {
          triggerGrowthAnimation(plantId, stageBefore, stageAfter, {
            x: plantAfter.x,
            y: plantAfter.y,
          });
  ...
  const completeTask = async (plantId: string) => {
    try {
      ...
      setSubmissionMessage(
        "Task completed! It will be auto-harvested in 6 hours."
      );
      setShowSuccessMessage(true);
  ...
  const completeTaskStep = async (
      ...
      prevPlants.map((plant) => {
        if (plant.id === plantId && plant.task_steps) {
          ...
          return {
            ...
            completion_date: isTaskCompleted
              ? new Date()
              : plant.completion_date,
          };
        }
        return plant;
      })
    ...
    try {
        ...
        prevPlants.map((plant) => {
          if (plant.id === plantId) {
            const updatedPlant = {
              ...
              experience_points:
                plant.experience_points + stepResult.experience_gained,
              shouldGlow: false,
              completed_steps: stepResult.completed_steps,
              total_steps: stepResult.total_steps,
              task_status: stepResult.task_completed
                ? "completed"
                : plant.task_status,
              completion_date: stepResult.task_completed
                ? new Date()
                : plant.completion_date,
            };
            return updatedPlant;
      ...
      if (stepResult.task_completed) {
        ...
        setTimeout(() => {
          const completedPlant = plants.find(
            (p) => p.id === plantId && p.task_status === "completed"
          );
          if (completedPlant) {
        ...
        setSubmissionMessage(
          `Step completed! ${stepResult.completed_steps}/${stepResult.total_steps} done`
        );
      }
      setShowSuccessMessage(true);
  ...
  const updateTaskStepPartial = async (
    plantId: string,
    stepId: string,
    hours: number
  ) => {
    ...
    try {
        ...
        prevPlants.map((plant) => {
          if (plant.id === plantId) {
            return {
              ...plant,
              experience_points:
                plant.experience_points + partialResult.experience_gained,
      ...
      setSubmissionMessage(
        `Progress added! +${partialResult.experience_gained} XP, ${hours}h logged`
      );
      setShowSuccessMessage(true);
      loadUserProgress();
    } catch (error) {
  ...
  const convertToMultiStep = async (
    plantId: string,
    ...
    try {
      ...
      setSubmissionMessage(
        `Task converted! Now has ${result.total_steps} steps to complete.`
      );
      setShowSuccessMessage(true);
      sounds.playUI("success");
    ...
    if (pendingPlantPosition) {
      try {
        ...
        const apiPlantData: PlantCreate = {
          ...
          productivity_category: plantData.category as
            | "work"
            | "study"
            | "exercise"
            | "creative",
  ...
  const shouldShowDailyTasks = useCallback(async () => {
    ...
    try {
      ...
      if (decayResult.decay_applied && decayResult.decay_applied > 0) {
        ...
        decayResult.message &&
        decayResult.message.includes(
          "No decay applied due to streak protection"
        )
      ) {
        setDailyDecayInfo(decayResult);
      } else {
  ...
  useEffect(() => {
    ...
    const backgroundMusicEnabled =
      bgMusicSetting === null ? true : bgMusicSetting === "true"; // Default to true
    const soundEnabled =
      localStorage.getItem("taskgarden_sound_enabled") !== "false";
  ...
  return (
    ...
    >
        ...
        {plantContextMenu && (
          ...
          >
            ...
            <div className="space-y-1">
              ...
              <button
                onClick={() =>
                  handlePlantAction("info", plantContextMenu.plant)
                }
              ...
              <button
                onClick={() =>
                  handlePlantAction("tasks", plantContextMenu.plant)
                }
                className="w-full text-left px-3 py-2 rounded-lg text-sm transition-colors flex items-center space-x-2 hover:bg-white/10 text-white"
              >
                <BarChart3 className="w-4 h-4" />
                <span>
                  {plantContextMenu.plant.stage >= 5
                    ? "Harvest Plant"
                    : "Log Work Hours"}
                </span>
              </button>
              ...
              <button
                onClick={() =>
                  handlePlantAction("manage", plantContextMenu.plant)
                }
                className="w-full text-left px-3 py-2 rounded-lg text-sm transition-colors flex items-center space-x-2 hover:bg-white/10 text-white"
              >
      ...
      {/* Animation Effects */}
      {Array.from(activeGrowthAnimations.entries()).map(
        ([plantId, animation]) => (
          <PlantGrowthEffects
            key={plantId}
            plantId={plantId}
